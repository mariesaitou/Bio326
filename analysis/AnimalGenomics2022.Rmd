...
The tutorial is adapted from [Nanopore Community](https://community.nanoporetech.com/knowledge/bioinformatics/nanopore-summary-statistic/tutorial
) (registration required) for educational purpose only.

## Aim of the tutorial.
Last time, you have started sequencing of pig genome using Nanopore MinION. 
Today, we will learn:
...
In this tutorial, we will investigate a pre-made pig genome sequence. You can change parameters to investigate your data for the report. Also, Matthew and I prepared some questions in each section. 
Please discuss and try the quizzes to deepen your understandig on Nanopore data.

You do not have to understand everything... the script is mainly separated in two parts, "extract the necessary data and organize it" and "data visualization". If you are ambitious, or you are already farmiliar with [**ggplot2**](https://www.r-graph-gallery.com/ggplot2-package.html), you can play around with the figures by adjusting parameters.
...
![](assets/Bio326-2022/Rconsole.png)



### Where is your personal directory? 
You have to **change** "working directory" to your directory (bio326-22-XX), usually every time you log in.
![](assets/Bio326-2022/prep4.png){width=80%}
```{r,eval=FALSE}
setwd("/net/cn-1/mnt/SCRATCH/bio326-22-XX/")  ## <-change it to your number
#setwd("/net/cn-1/mnt/SCRATCH/bio326-21/bio326_2022/AnimalGenomics/QCTutorial/") ... for Marie's trial

## Install necessary libraries ... only the first time you try the analysis
install.packages(c("dplyr","emojifont","extrafont","ggplot2","RColorBrewer","tufte","caTools","fastmatch"))

## Load libraries ... usually every time you open the environment
## but you can also save and load your working envidonment by
...
# Make a result directory ... only the first time you try the analysis
...
```{r, eval=FALSE}
## inputFile. You can select your file. See below.
...
pig92 (flowcellId: PAG06497)
"/mnt/SCRATCH/bio326-21/bio326_2022/AnimalGenomics/92/sequencing_summary_92.txt"

pig17 (flowcellId: PAG04717)
"/mnt/SCRATCH/bio326-21/bio326_2022/AnimalGenomics/17/sequencing_summary_17.txt"

From the real experiments


group4　(flowcellId: FAL92123)
"/mnt/SCRATCH/bio326-21/bio326_2022/AnimalGenomics/group4/sequencing_summary_FAL92123_033c66cb.txt"

group6　(flowcellId: FAL90662)
"/mnt/SCRATCH/bio326-21/bio326_2022/AnimalGenomics/group6/sequencing_summary_FAL90662_502cb47c.txt"

group1 (flowcellId: FAL92096) ... which failed in basecalling. Only Figure2 can be properly generated.
"/mnt/SCRATCH/bio326-21/bio326_2022/AnimalGenomics/group1/sequencing_summary_FAL92096_12611e97.txt"
...
```{bash, eval=FALSE}
more /net/cn-1/mnt/SCRATCH/bio326-21/bio326_2022/AnimalGenomics/92/sequencing_summary_92.txt
```
...
You have to **change** "output file name" if you work with multiple dataset not to overwrite previous output.
```{r, eval=FALSE}
...
```{bash, eval=FALSE}
scp yourID@login.orion.nmbu.no:/file.png your/local/file.png 
```
...
<details>
  <summary>  **Matthew's hint** </summary>      
...
You have to **change** "output file name" if you work with multiple dataset not to overwrite previous output.
```{r,eval=FALSE}
ggsave(file.path("Results", "Figure3.length.png"), plot=length, device="png", units="cm", width=12, height=6, dpi=reportDPI)
```
...
# Time to explore
Now you have investigated one sample. 
You can investigate another sample, and your own three flowcells. 
To do so, **change** input file name, flowcell info, and output file names.
